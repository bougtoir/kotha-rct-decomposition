...
§2でグラフラプラシアンの基礎を復習し、§3で磁気ラプラシアンを導入する。§4でスペクトル因果性を厳密に定式化し（§4.1.1 でデータ駆動の非対称統計量DPIを導入する）、§5でHodge分解との関係を示す。§6で既存手法との比較、§7で関連研究・先行文献・周辺調査を概観する。§8で実データ（UCI心疾患データ）への適用例を示し、§9–§12でLiNGAMとの構造比較、ECDアンサンブル、DAG転移点、循環プルーニングを詳述する。§13で理論的課題と展望を議論する。
...
### 11.1 $\alpha$ 掃引実験：DPI導入による滑らかな相転移

§4.1.1 で導入したDPIをデータ駆動成分に用いた場合、$\alpha$ を $0 \to 1$ まで掃引した結果（$C =$ 臨床知識行列）、DAG度（$r_{\text{gradient}}$）は**滑らかに増加**する：
...
*図8: DPIベースのモデルにおける $\alpha$ 掃引。(A) $r_{\text{gradient}}$ は $\alpha = 0$ で 0.581 から始まり、ドメイン知識の増加とともに滑らかに 0.859 へ到達。(B) 辺数とLiNGAM一致率。(C) 非対称ノルム。(D) 位相図: $\alpha = 0$ が原点ではなく有限の値を持つ（DPIの方向性信号）。*

**旧モデルとの対比**: 旧モデル（$|\rho|$ ベース）では $\alpha = 0$ で $r_{\text{gradient}}$ が未定義（0/0）となり、$\alpha > 0$ で即座に 0.859 に到達する不連続相転移を示していた。これは

$$A_{\text{old}}(\alpha) = \alpha \cdot (C - C^T) + (1-\alpha) \cdot \underbrace{(|\rho| - |\rho|^T)}_{= 0} = \alpha \cdot (C - C^T)$$

において $r_{\text{gradient}}$ がスカラー倍不変であるためであった。DPIの導入により、データ駆動成分 $D_{\text{DPI}} - D_{\text{DPI}}^T \neq 0$ が $\alpha = 0$ でも非自明な方向性信号を提供するため、$\alpha$ が連続的な改善パラメータとして機能するようになった。
...
$\alpha$ は外部磁場 $h$ に相当する。DPI導入前は $h > 0$ であれば無限小でも秩序化を誘起していた（不連続相転移）が、DPI導入後は $h = 0$ でも部分的な秩序状態（$r_{\text{gradient}} = 0.581$）が存在し、$h$ の増加とともに秩序が連続的に強化される。これは物理学における**ソフト強磁性相**に相当し、DPIのデータ駆動的方向性信号が「残留磁化」のような役割を果たしていると解釈できる。

### 11.6 三つの閾値のまとめ

| 閾値 | 値 | 意味 |
|---|---|---|
| $\alpha^*$（知識量閾値） | **$0^+$（DPIにより連続化）** | DPIにより $\alpha = 0$ でも $r_{\text{gradient}} = 0.581$; ドメイン知識で滑らかに向上 |
