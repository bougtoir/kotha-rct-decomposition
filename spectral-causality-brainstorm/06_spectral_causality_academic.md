...
UCI心疾患データにおいて、Hodge因果ポテンシャル $\phi$ と臨床的介入可能性（interventionability）の間に顕著な対応が観察される：

| 変数 | $\phi$（上流度） | 介入可能性 $\iota$ | 臨床的理由 |
|---|---|---|---|
| Age | 最大（最上流） | **不可能**（$\iota = 0$） | 不可逆な生物学的プロセス |
| MaxHR | 上流 | **困難**（$\iota \approx 0.3$） | 加齢・体質に依存 |
| STDep | 中流 | **間接的**（$\iota \approx 0.5$） | PCI/CABGで虚血改善 |
| Chol | 下流 | **容易**（$\iota \approx 0.9$） | スタチン |
| RestBP | 最下流 | **容易**（$\iota \approx 0.8$） | 降圧薬 |

この対応は偶然ではなく、構造的理由がある。介入不可能な変数は**外生的（exogenous）**——他の変数に影響されず一方的に影響を与えるため、DAGの根（root）に位置する。因果推論の用語では、構造方程式 $X_i = f_i(\text{parents}(X_i), \varepsilon_i)$ において $\text{parents}(X_i) = \emptyset$ の変数が最上流であり、これは定義上介入不可能な変数に対応する。

**命題 8.1（非形式的）**: $\phi(i)$ と $\iota(i)$ の間に負の相関が成立する条件は、(a) 介入不可能な変数がDAGの根に位置し、(b) 介入可能な変数ほど多くの親を持つ場合である。医療データではこの条件が自然に満たされやすい。

### 8.6 DAG転移点：ドメイン知識の「量」より「質」

§8.2で述べたハイブリッド手法において $\alpha$ の選択が重要であるように見えるが、実験的に以下が判明した：

**定理的知見 8.2（数値実験に基づく）**: UCI心疾患データにおいて、$r_{\text{gradient}}(\alpha)$ は $\alpha = 0$ で未定義（対称ユーティリティ行列では方向性フローがゼロ）、$\alpha > 0$ で即座に 0.859 に到達し、$\alpha$ の値に依存しない。

これはHodge分解の勾配比率がフローの**構造**（方向の符号パターン）のみに依存し、**大きさ**（$\alpha$ によるスケーリング）には依存しないためである：

$$A(\alpha) = \alpha(C - C^T) + (1-\alpha)\underbrace{(|\rho| - |\rho|^T)}_{= 0} = \alpha(C - C^T)$$

$r_{\text{gradient}} = \|\nabla\phi\|^2 / \|\omega\|^2$ はフロー $\omega$ のスカラー倍に不変。

**真の閾値は知識の品質 $p_{\text{flip}}$**: ドメイン知識の辺方向を $p_{\text{flip}}$ の割合で反転させた実験（200試行）では、$p_{\text{flip}}^* \approx 0.15$（85%以上正しい）で $r_{\text{gradient}} > 0.5$。$p_{\text{flip}} \approx 0.3$ で最低点（U字型カーブ）——**部分的な誤情報は完全な無知より悪い**。

### 8.7 ECDアンサンブルと実運用パイプライン

上記の知見を統合すると、以下の **ECD（Ensemble Causal Direction）パイプライン** が実運用に最適である：

$$U_{\text{ECD}}(i \to j) = \alpha \cdot C_{\text{LiNGAM}}(i,j) + (1-\alpha) \cdot |\text{corr}(X_i, X_j)|$$

1. **LiNGAMでDAG推定**（ドメイン知識不要）
2. **ブートストラップ安定性でプルーニング**（安定性 > 80% の辺のみ残す）
3. **残った辺を $C_{\text{LiNGAM}}$ として低α（0.01–0.1）で設定**
4. **Hodge分解でDAG/DCGハイブリッド解析**

低αが十分である理由は§8.6の通り。LiNGAMによるプルーニングはブートストラップ安定性を通じて $p_{\text{flip}}$ を間接的に制御する（不安定な辺 ≈ 方向不確実 ≈ 高 $p_{\text{flip}}$ リスク）。

循環構造のプルーニングについては、辺ごとの**フィードバック率**（$\text{curl}^2 / \text{total}^2$）に基づき、分析目的に応じた粒度で因果モデルを導出できる。UCI心疾患データでは MaxHR ↔ STDep のフィードバック率が73%と高く、DAG仮定下での一方向因果（LiNGAM）は臨床的フィードバックループを見落とす可能性がある。
...
4. **$p_{\text{flip}}$ の自動推定**: LiNGAMとの因果方向一致率からドメイン知識品質を推定するメタ手法
5. **プルーニング閾値の自動化**: フィードバック率のブートストラップ信頼区間に基づく統計的閾値設定
