...
8. [LiNGAM vs スペクトル因果性：構造比較](#8-構造比較)
9. [ECDアンサンブルと因果上流性](#9-ecdアンサンブルと因果上流性)
10. [DAG転移点の解析](#10-dag転移点の解析)
11. [循環構造のプルーニングと実運用](#11-循環構造のプルーニングと実運用)
12. [理論的課題と展望](#12-理論的課題と展望)
...
## 8. LiNGAM vs スペクトル因果性：3条件での構造比較

§7で個別に示した各手法の結果を、同一データ上で体系的に比較する。以下の3条件で因果構造を推定した：
...
### 9.1 ECD（Ensemble Causal Direction）パイプライン

§8でLiNGAM単独とスペクトル因果性単独の限界が明らかになった。自然な問い：**LiNGAMの推定結果をドメイン知識として使えばどうなるか？**
...
**命題 9.1（非形式的）**: $\phi(i)$ と $\iota(i)$ の間に負の相関が成立する条件は、(a) 介入不可能な変数がDAGの根に位置し、(b) 介入可能な変数ほど多くの親を持つ場合である。
...
### 10.3 Leave-One-Edge-Out：根ノードの方向性が骨格
...
**なぜ低αで十分か**: §10.1で証明されたように、$r_{\text{gradient}}$ は $\alpha$ のスカラー倍に不変。低αのメリットは**データ駆動の相関重みが辺の太さに反映**されること。$\alpha = 0.01$ なら「方向はドメイン知識、強さはデータ」という自然な分担。
...
**予想 12.1** 以下の条件下で、SCD は因果方向と一致する：
...
(d) **ハイブリッド**: $U(i,j) = \alpha \cdot U_{\text{expert}}(i,j) + (1 - \alpha) \cdot U_{\text{data}}(i,j)$。本稿ではこの手法を用いた（§7: $\alpha = 0.6$）。ただし§10で示したように $\alpha$ の値自体は結果に影響しない。
...
4. **$p_{\text{flip}}$ の自動推定**: LiNGAMとの因果方向一致率からドメイン知識品質を推定するメタ手法（§10.2の$p_{\text{flip}}^*$を実データ上で自動算出）
5. **プルーニング閾値の自動化**: フィードバック率のブートストラップ信頼区間に基づく統計的閾値設定（§11.3）
6. **ドメイン知識のエンコード改善**: 「情報的影響度」ではなく「介入的因果強度」をエンコードする方法の開発（§8.2の乖離を解消）
