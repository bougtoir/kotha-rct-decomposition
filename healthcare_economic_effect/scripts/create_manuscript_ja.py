...
# ---------------------------------------------------------------------------
# References (Vancouver style, ordered by first appearance)
# ---------------------------------------------------------------------------
REFERENCES = [
    # 1
...
def build_ja_docx():
    ...
    add_para(doc,
        "日本政府は2005年の「骨太の方針」以来、国民医療費の伸びの抑制を政策目標としてきた{1}。"
        "2019年の日本の対GDP保健医療支出は11.0%でOECD38か国中第5位であり{10}、"
        "高齢化の進行とともに医療費の「持続可能性」は中心的な政策課題となっている{11}。"
    )
    add_para(doc,
    ...
    add_para(doc,
        ...
        "公共事業（2.1〜2.5）や電力・ガス（1.8〜2.0）と同等以上である{1,2}。"
        "米国のMedicare乗数1.7{3}が低いのは、"
        "高い薬価と管理コストが域外（海外製薬）に漏出するためと考えられる。"
    )

    add_heading(doc, "2.2 供給側：健康資本と人的資本", level=2)
    add_para(doc,
        "Mushkin（1962）以来、健康は人的資本の構成要素として認識されてきた{8}。"
    ...
    add_table_from_df(doc, hlgh_display, "表2. Health-Led Growth Hypothesisの実証研究要約")

    add_para(doc,
        "全ての研究が医療支出→GDP成長の正の効果を確認しており、"
        "多くで双方向因果（bidirectional causality）が検出されている{4-7}。"
        "これは医療費が単なるコストではなく、"
    ...
    add_table_from_df(doc, sust_display,
                      "表3. ニュートラル財政持続可能性指標（9か国）")

    add_figure(doc, os.path.join(FIG, "fig3_fiscal_sustainability.png"),
               "図3. 各国の医療支出財政回収率（τ·m / pf）")

    add_para(doc,
        "9か国中3か国（日本1.09、フランス1.18、スウェーデン1.04）で"
        "需要側のみの財政回収率が1.0を上回った。"
        "残りの6か国も0.76〜0.96と1.0に近接しており、"
    ...
    add_figure(doc, os.path.join(FIG, "fig2_che_vs_lifeexp.png"),
               "図2. 医療支出（対GDP%）と平均寿命の関係（OECD, 2019年）")
    ...
    add_para(doc,
        ...
        "需要側のみでも日本・フランス・スウェーデンでは税収還流が公的支出を上回り、"
        "他の6か国も0.76〜0.96と大部分を回収している。"
...
def build_ja_pptx():
    ...
    figures = [
        ("fig1_io_multipliers.png",
         "図1. 各国の医療セクターI-O乗数の比較",
         "各国の医療支出の産業連関乗数。日本は2.78倍でOECD諸国中最大。出典: Yamada & Imanaka 2015他"),
        ("fig2_che_vs_lifeexp.png",
         "図2. 医療支出（対GDP%）と平均寿命（OECD, 2019年）",
         "横軸: 対GDP医療支出割合(%), 縦軸: 出生時平均寿命(年). 二次関数フィットは逓減的関係を示す。"),
        ("fig3_fiscal_sustainability.png",
         "図3. 各国の医療支出財政回収率（τ·m / pf）",
         "財政回収率 = (実効税率 × I-O乗数) / 公的負担割合。1.0以上は税収還流が公的支出を上回ることを意味する。"),
