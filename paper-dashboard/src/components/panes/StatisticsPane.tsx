...
export function StatisticsPane({ paper }: Props) {
  ...
  const StatCard = ({ value, valueField, label, limit, limitField }: {
    value: number; valueField: StatField; label: string; limit?: number; limitField: StatField;
  }) => {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
      <StatCard value={s.wordCount} valueField="wordCount" label="Word Count" limit={s.wordLimit} limitField="wordLimit" />
      <StatCard value={s.figureCount} valueField="figureCount" label="Figures" limit={s.figureLimit} limitField="figureLimit" />
      <StatCard value={s.tableCount} valueField="tableCount" label="Tables" limit={s.tableLimit} limitField="tableLimit" />
      <StatCard value={s.referenceCount} valueField="referenceCount" label="References" limit={s.referenceLimit} limitField="referenceLimit" />
    </div>
  );
}
