...
add_para(doc,
    "合成軌跡は再統一時点（1990年）で観測人口を3.7%下回り、2023年までに16.2%に拡大する。"
    ...
    "収束。合成東西モデルの全体MAPE 6.6%は、主要分析（表1）におけるドイツの比較的低い"
...
add_para(doc,
    ...
    "スロバキアのMAPE 10.1%を達成する。スロバキアのより大きな誤差はEU加盟（2004年）後の"
...
add_para(doc,
    ...
    "後継国間で大きく異なる：クロアチア（MAPE 4.1%）と北マケドニア（6.1%）は合理的な"
    "適合度を示し、比較的安定した紛争後の人口動態を反映する。ボスニア・ヘルツェゴビナ"
    "（7.7%）とスロベニア（12.3%）はより大きな誤差を示す——ボスニアは戦争関連の人口損失"
    "と避難、スロベニアは小規模なEU加盟国としての移民流入による成長が原因。セルビア"
    "（7.2%）とモンテネグロ（8.4%）はその中間に位置する。",
    size=11, space_after=12)
...
add_para(doc,
    ...
    "著しい流出と西欧への移民（特に2004年のEU加盟後）が続いた。モデルMAPEは4.9%"
...
add_para(doc,
    ...
    "性能は顕著に異なる：エチオピア（MAPE 16.7%）は実質的な過大推計を示し、10年ごとの"
    "パラメータ更新が捕捉する以上に速い10年内の出生率低下を反映する。エリトリア"
    "（MAPE 37.9%）は自然実験サンプルで最大の誤差を示し、長期の徴兵制度、紛争関連の"
...
c2_data = [
    ['ドイツ（合成東西）', '再統一（1990）', '6.6', '移民＋国内移動'],
    ['チェコ', 'ビロード離婚（1993）', '6.3', 'EU後の移民流出（穏やか）'],
    ['スロバキア', 'ビロード離婚（1993）', '10.1', 'EU・西欧への移民流出'],
    ['クロアチア', 'ユーゴ解体（1991）', '4.1', '紛争後の安定化'],
    ['スロベニア', 'ユーゴ解体（1991）', '12.3', '移民流入（EU加盟国）'],
    ['ボスニア', 'ユーゴ解体（1991）', '7.7', '戦争関連の避難'],
    ['セルビア', 'ユーゴ解体（1991）', '7.2', '難民流、移民流出'],
    ['北マケドニア', 'ユーゴ解体（1991）', '6.1', '穏やかな移民効果'],
    ['モンテネグロ', 'ユーゴ解体（1991）', '8.4', '小国、変動的なフロー'],
    ['エストニア', 'ソ連解体（1991）', '4.9', 'ロシア系住民の流出'],
    ['ラトビア', 'ソ連解体（1991）', '6.6', '移民流出（民族＋EU）'],
    ['リトアニア', 'ソ連解体（1991）', '7.1', '持続的な移民流出'],
    ['エチオピア', 'エリトリア独立（1993）', '16.7', '急速な出生率低下'],
    ['エリトリア', 'エリトリア独立（1993）', '37.9', '紛争、徴兵、移民流出'],
]
...
add_para(doc,
    ...
    "エストニア（4.9%）、チェコ（6.3%）はいずれもこの範囲に収まり、カンタム・テンポ・生存"
    "メカニズムが政治的不連続性の文脈でも人口動態の大部分を捕捉することを示す。\n\n"
    "第二に、モデルと観測の乖離の大きさは移民成分の直接的な推定を提供する。ドイツの2023年"
    "までの16.2%の合成・観測ギャップは、移民が自然増加だけでは生じなかったであろう約1,350万人"
    "——総人口の約6分の1に相当——を追加したことを意味する。この定量化は閉鎖モデルが自然な"
    "人口動態を分離するからこそ可能である。\n\n"
    "第三に、モデルの限界は紛争影響下・データ希少な環境（エリトリア：37.9%）および持続的な"
    "移民流出が人口の大きな割合を除去する場合（スロベニア：12.3%、スロバキア：10.1%）において"
