...
def _condense_abstract_results(results):
    """Shorten the CCTC Results paragraph while preserving all computed numbers."""
    s = re.sub(r'\bodds ratio \(OR\)', 'OR', results, flags=re.I)
    s = re.sub(r'\bhazard ratio \(HR\)', 'HR', s, flags=re.I)
    s = re.sub(r'\bin the magnesium case,\s*', '', s, flags=re.I)
    s = re.sub(r'\bin the statins case,\s*', '', s, flags=re.I)
    s = re.sub(r'\bwith an event-rate ratio of\s*', 'event-rate ratio=', s, flags=re.I)
    s = re.sub(r'\bBayesian integration \(alpha = ', r'Bayesian integration ($\\alpha$=', s)
    s = re.sub(r'\byielded\b', 'gave', s, flags=re.I)
    s = re.sub(r'\bboth probabilities remained below conventional decision thresholds, leaving conclusions uncertain\b',
               'both remained below decision thresholds, leaving conclusions uncertain', s, flags=re.I)
    s = re.sub(r'\$I\^2\$\s*=\s*', r'$I^2$=', s)
    # Restore sentence-case capitalization after removed leading phrases
    s = re.sub(r'^(\*\*Results\*\*:\s*)control', r'\1Control', s, flags=re.I)
    s = re.sub(r'([.?!;]\s*)observational HR', r'\1Observational HR', s, flags=re.I)
    return s.strip()
...
def _reframe_abstract(md):
    ...
    new = (
        "**Background/Aims**: Randomized controlled trials (RCTs) remain central to drug "
        "development, but meta-analyses can become informationally insufficient when enrollment "
        "shifts patients to lower-risk profiles. KOTHA is a simulation-based framework that "
        "quantifies structural information loss and integrates RCT and observational evidence through "
        "power-prior Bayesian synthesis.\n\n"
        "**Methods**: KOTHA has three modules: counterfactual power simulation, Bayesian evidence "
        "integration with design-bias discounting, and GRADE-compatible interpretation. We applied "
        "it to magnesium in acute myocardial infarction and statins in heart failure and evaluated "
        "operating characteristics in a prespecified simulation study.\n\n"
        f"{results}\n\n"
        "**Conclusions**: KOTHA distinguishes evidence of no effect from no evidence of effect and "
        "provides a transparent diagnostic for trial design.\n\n"
...
def _new_intro():
    ...
    return (
        ...
        "An alternative structural explanation is particularly relevant for Phase II/III "
        ...
        "become informationally insufficient. We call this phenomenon **structural information "
        ...
        "The concepts of optimal information size (OIS) and trial sequential analysis (TSA) "
        "provide partial remedies. OIS recognizes that meta-analyses, like individual trials, "
        ...
        "framework tightly links prospective power assessment, retrospective diagnostic "
