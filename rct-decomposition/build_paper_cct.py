...
The concepts of optimal information size (OIS) and trial sequential analysis (TSA) provide partial remedies. OIS recognizes that meta-analyses, like individual trials, require a minimum information size to reach reliable conclusions {r('Pogue', 'Wetterslev')}. TSA applies sequential monitoring boundaries to cumulative meta-analysis, distinguishing evidence of no effect (futility boundary crossed) from no evidence of effect (boundary not crossed) {r('Brok', 'TSAManual')}. However, these tools are underused in guideline development, and neither directly quantifies the information loss resulting from enrollment-driven risk-profile shifts.
...
The KOTHA Framework integrates counterfactual power simulation, Bayesian evidence synthesis, and structured GRADE interpretation to diagnose structural information loss in RCT meta-analyses. In the magnesium case, Module K identified a {v['mg_rate_reduction_pct']:.0f}% temporal decline in control event rates from the pre-thrombolysis era to ISIS-4. The divergence between pre-ISIS-4 and all-trials estimates is more consistent with era-dependent treatment effect heterogeneity ($I^2$ = {mg_all_i2}%) than with underpowering alone. In the statins case, Module K showed that RCT populations had roughly half the event rate of observational cohorts, and that power for a modest effect (HR = 0.85) dropped from {v['st_power_s1_hr085']:.0f}% to {v['st_power_s2_hr085']:.0f}%. Module T demonstrated that even modest borrowing from observational evidence shifted posterior probabilities of benefit substantially, but uncertainty remained. Module H classified magnesium as {v['mg_kotha_classification'].lower()}, and statins as {v['st_kotha_classification'].lower()}, rather than "evidence of no effect."
