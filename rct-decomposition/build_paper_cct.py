...
**Results**: In the magnesium case, control event rates fell from {v['s1_rate']*100:.1f}% to {v['s2_rate']*100:.1f}%; the pre-ISIS-4 pooled odds ratio (OR) was {mg_pre_or:.2f} (95% CI: {mg_pre_lo:.2f}-{mg_pre_hi:.2f}) and the all-trials estimate {mg_all_or:.2f} ({mg_all_lo:.2f}-{mg_all_hi:.2f}) with $I^2$ = {mg_all_i2}%. In the statins case, observational hazard ratio (HR) was {st_obs_hr:.2f} ({st_obs_lo:.2f}-{st_obs_hi:.2f}) versus RCT HR {st_rct_hr:.2f} ({st_rct_lo:.2f}-{st_rct_hi:.2f}), with an event-rate ratio of {v['st_rate_ratio']:.2f}. Bayesian integration (α = {v['alpha_example']:.1f}) yielded OR {v['mg_pp_alpha3_or']:.2f} (CrI {v['mg_pp_alpha3_lo']:.2f}-{v['mg_pp_alpha3_hi']:.2f}), P(OR < 1) = {v['mg_pp_alpha3_p1']:.1f}% for magnesium and HR {v['st_pp_alpha3_hr']:.2f} ({v['st_pp_alpha3_lo']:.2f}-{v['st_pp_alpha3_hi']:.2f}), P(HR < 1) = {v['st_pp_alpha3_p1']:.1f}% for statins; both probabilities remained below conventional decision thresholds, leaving conclusions uncertain.
