...
def create_manuscript_ja():
    ...
    intro_paras = [
        ...
        ("本研究の着想は、著者らが実施したプレストン曲線の予備的解析に端を発する。{6}"
         ...
         "実際、CO2排出に関する環境クズネッツ曲線は繰り返し異議を唱えられており、{7,8}"
         "ダニング・クルーガー効果は平均への回帰という統計的アーティファクトであるという批判がある。{9}"),

        ("しかし、これらの個別的な指摘にもかかわらず、学際的な体系的監査は行われてこなかった。本研究は、"
    ...
    doc.add_paragraph(
        "52本の定説曲線の選定は、代表性、再現可能性、科学的妥当性を確保するための"
        "体系的プロトコルに基づいて行った。目的は全ての非線形関係の網羅的列挙ではなく、"
        "分野・データ構造・非線形性の種類の多様性を捕捉する戦略的標本の選定である。"
    )

    p = doc.add_paragraph()
    run = p.add_run('包含基準. ')
    run.bold = True
    p.add_run("以下の5条件を全て満たす曲線を対象とした：")

    inclusion_ja = [
        "（1）命名的・定説的地位：当該関係が特定の名称（例：「フィリップス曲線」「クズネッツ曲線」"
        "「クライバーの法則」）で広く知られていること。操作的には、当該分野の主要教科書2冊以上または"
        "総説論文に記載されていること、もしくは元の定式化がGoogle Scholar被引用数500以上を有すること。",
        "（2）非線形性の主張：凹性、凸性、逆U字、U字、J字、S字、べき乗則、指数関数、対数関数など"
        "の非線形関数形が理論の中心的特徴として明示的に主張されていること。純粋な線形関係は、"
        "非線形拡張が文献で広く議論されている場合にのみ含めた。",
        "（3）二変量検証可能性：中心的主張がモデル比較に適した二変量関係 y = f(x) として"
        "表現可能であること。多変量理論は、二変量射影が標準的な教科書表現である場合に含めた"
        "（例：クズネッツはコントロールなしで所得 vs ジニをプロット）。",
        "（4）データ利用可能性：線形モデルと二次（または適切な非線形）モデルの両方を"
        "N ≥ 10の観測値で推定するのに十分な公開データが存在すること。データソースには"
        "世界銀行WDI、OECD.Stat、Penn World Tables、国連人口部、USGS地震カタログ、"
        "公表されたメタ分析、および元論文のデジタル化データを含む。",
        "（5）政策的・理論的関連性：曲線の形状（単なる相関の存在ではなく）が理論または政策に"
        "実質的含意を持つこと。例えば、ラッファー曲線の頂点位置は最適税率政策を決定し、"
        "EKCの形状は成長のみで汚染が解決されるかを決定する。",
    ]
    for criterion in inclusion_ja:
        doc.add_paragraph(criterion, style='List Bullet')

    p = doc.add_paragraph()
    run = p.add_run('除外基準. ')
    run.bold = True
    p.add_run("以下に該当するものは除外した：")

    exclusion_ja = [
        "純粋に定義的・恒等的な曲線（例：ローレンツ曲線は図示上の恒等式）。",
        "独立した再現に利用できない非公開データを必要とする関係。",
        "非線形の主張が時間的ダイナミクスのみに関するもの（安定的な横断面的またはパラメトリック形式を持たない景気循環の形状等）。",
        "当該分野で既に正式に撤回されたか普遍的に放棄された関係。",
        "二変量射影が無意味であることが認められている多変量指定を必要とする曲線（例：資本と労働の両方を必要とする生産関数）。",
    ]
    for criterion in exclusion_ja:
        doc.add_paragraph(criterion, style='List Bullet')

    p = doc.add_paragraph()
    run = p.add_run('分野別層別化. ')
    run.bold = True
    p.add_run(
        "特定分野の過剰代表を避けるため、8つの主要分野にわたって選定を層別化した："
        "経済学（12本）、公衆衛生・疫学（10本）、人口学（6本）、環境科学・生態学（6本）、"
        "心理学・行動科学（5本）、物理学・自然科学（4本）、政治学・社会学（5本）、"
        "農学・栄養学（4本）。各分野内では、(a) 政策文書での引用頻度、"
        "(b) 学部教科書での存在、(c) 関係の妥当性に関する活発な学術的議論の存在で優先順位を付けた。"
    )
    ...
    run.bold = True
    p.add_run(
        "候補曲線は以下を通じて同定した：(1) 各分野の主要ハンドブック（例：Handbook of Economic Growth、"
        "Oxford Handbook of Health Economics、Demographic Methods）における「命名された法則」"
        "と「定型化された事実」の系統的レビュー、(2) 各分野のトップ5ジャーナルにおけるタイトルに"
        "「curve」「law」「paradox」を含む論文の引用分析、(3) 経済学・心理学・生態学における"
        "再現性と頑健性に関する先行メタ科学的研究のレビュー、(4) 経験的根拠が疑問視されている"
        "曲線を同定するための「再現性危機」に関する既存文献の参照。これにより初期候補78本が得られ、"
        "そのうち52本が全ての包含基準を満たし除外基準に該当しなかった。"
    )

    p = doc.add_paragraph()
    run = p.add_run('非線形性主張の類型. ')
    run.bold = True
    p.add_run(
        "選定した52本は5つの異なる非線形性の類型を代表する："
        "(a) 逆U字・山型（クズネッツ、ラッファー、ヤーキーズ・ドッドソン等）：14本、"
        "(b) U字・J字型（BMI死亡率、飲酒死亡率、バーカー等）：8本、"
        "(c) 凹型・飽和型（プレストン、エンゲル、ミッチェルリッヒ等）：12本、"
        "(d) べき乗則・対数線形（ジップ、クライバー、グーテンベルグ・リヒター等）：8本、"
        "(e) S字型・構造変化（人口転換、ムーアの法則等）：10本。"
        "この類型的多様性により、特定の関数形の検出（または非検出）に偏らない再検証が可能となる。"
    )
