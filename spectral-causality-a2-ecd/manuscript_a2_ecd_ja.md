...
LiNGAM（DAG）、スペクトル因果性（$\alpha = 0.6$）、DPI（$\alpha = 0$）で推定された因果方向の比較（図4）：
...
スペクトル因果性がLiNGAMと逆方向を示唆する場合（図5）、それは**必ずしも誤りではない**。両手法は異なる種類の因果性を捕捉している：
...
正しいドメイン知識の辺方向の割合$p_{\text{flip}}$を系統的に反転する（200試行、$\alpha = 0.6$；図7）：
...
DAGは数学的に便利だが、臨床的には循環モデルの方がしばしば正確である。ECDフレームワークはDAGを強制しない；代わりに各辺のフィードバック度合いを**定量化**し（図6）、臨床医がどの循環を保持するかについて情報に基づく判断を可能にする。
