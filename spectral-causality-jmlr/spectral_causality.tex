...
to accumulate phase differently from downstream nodes.  For path graphs,
we show in Lemma~\ref{lem:path-phase} that the eigenequation recursion
forces a strictly positive phase increment $\Delta\theta>0$ per directed
edge, yielding monotone phase ordering along causal paths.
Theorem~\ref{thm:tree-phase} extends this to tree DAGs via structural
induction on tree depth, showing that the parent-to-child phase advance
persists across branches.  The full proof is given in
Appendix~\ref{app:phase-proof}.\hfill$\square$
