...
\citealp{Sachs2005}), and the UCI Heart Disease dataset ($N=297$, 5
...
\citealp{Shimizu2011}), the PC algorithm, GES, and NOTEARS.  On the
Sachs network, spectral causality with partial domain knowledge (5 of
17 edges) achieves TPR~$0.56$ and FDR~$0.27$, outperforming NOTEARS,
while providing a unique DAG-adequacy diagnostic via
$r_{\mathrm{gradient}}$.  Hodge decomposition reveals clinically
meaningful feedback loops invisible to DAG-based methods.  We propose
the \emph{Ensemble Causal Direction} (ECD) pipeline that integrates
LiNGAM's identifiability guarantees with spectral causality's feedback
quantification, achieving comprehensive coverage of Hill's nine
...
We prove this in two stages in Appendix~\ref{app:phase-proof}.
...
\emph{Stage~2} (Theorem~\ref{thm:tree-phase}): For tree DAGs, we use
...
We construct $C_{\mathrm{domain}}$ from established cardiovascular
...
We construct the utility directed graph (Eq.~\ref{eq:utility-hybrid}) with
...
We generate random DAGs with $n\in\{5,10,20\}$ nodes and expected
...
\item[\emph{(ii)}] \textbf{PC algorithm} \citep{Spirtes2000}: constraint-based,
  using partial correlations with significance level $\alpha_{\mathrm{PC}}=0.05$.
\item[\emph{(iii)}] \textbf{GES} \citep{Chickering2002}: score-based
  greedy search with BIC scoring.
\item[\emph{(iv)}] \textbf{NOTEARS} \citep{Zheng2018}: continuous
  optimization with acyclicity constraint.
\item[\emph{(v)}] \textbf{Spectral (DPI, $\alpha=0$)}: our method with
...
decades of biological experimentation.  We use the observational subset
($N=853$, anti-CD3/CD28 stimulation) to match the cross-sectional
setting of our framework.

\paragraph{Domain knowledge.}
Since our framework can incorporate external knowledge, we construct
...
spectral causality with domain knowledge~(B, $\alpha=0.6$), and
spectral causality with DPI alone~(C, $\alpha=0$)---for all
$\binom{5}{2}=10$ variable pairs.
...
The practical utility of DPI depends on finite-sample behavior.  We
...
\emph{Diagnostic marker reversals.}  For pairs such as
...
upstream variable (cause) in clinical practice.  Cholesterol levels
inform about cardiovascular risk (not the reverse), and blood pressure
predicts exercise capacity.  This pattern suggests that the spectral
framework's ``informational direction'' captures the predictive structure
of clinical reasoning, which is complementary to---not contradictory
with---the interventional structure that LiNGAM recovers.

\emph{Feedback detection.}  The 73\% curl component on the
MaxHR$\leftrightarrow$STDep edge is clinically meaningful: exercise
intolerance $\to$ ischemia $\to$ increased myocardial oxygen demand
$\to$ further exercise limitation constitutes a well-known pathological
feedback loop.  DAG-based methods force a unidirectional
MaxHR$\to$STDep edge, losing this bidirectional structure.  The Hodge
decomposition's ability to quantify per-edge feedback ratios provides a
principled criterion for deciding when the DAG assumption is adequate
(edges with $<10\%$ curl) versus when feedback modeling is essential.

The smooth $\alpha$-sweep from $r_{\mathrm{gradient}}=0.581$ at
$\alpha=0$ to $0.859$ at $\alpha=1$ demonstrates that domain knowledge
and data-driven inference provide complementary rather than redundant
information.  The fact that DPI alone ($\alpha=0$) already achieves
67\% directional agreement with LiNGAM validates the DPI as a
meaningful asymmetric statistic, while the additional 20+ percentage
points from domain knowledge highlight the value of expert input when
...
phases rather than a monotone function.  Preliminary numerical experiments
suggest the correspondence holds approximately for sparse DAGs with
bounded in-degree, but a formal proof is lacking.
...
Our experiments use $q=0.25$ for eigenvector analysis and $q=0.15$ for
...
We prove Theorem~\ref{thm:phase-direction} in two stages: first for
...
\label{eq:path-recursion}
  ...
  + \frac{iw}{d_{v_k}}\,u(v_{k+1}),
\end{equation}
where we suppress the eigenvector index and write $u=u_2$.  Writing
...
$\Delta\theta>0$, we have $\theta_2(v_1)<\theta_2(v_2)<\cdots<\theta_2(v_n)$.
\end{proof}
...
\[
  ...
  = \frac{-i}{\sqrt{m}}\,u(r).
\]
Writing $u(r)=r_0 e^{i\theta_0}$, we obtain
