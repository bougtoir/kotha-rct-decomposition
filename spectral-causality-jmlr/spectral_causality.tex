...
\caption{Complex Causal Index~$\CCI(i,j)$ for all variable pairs plotted in
  the complex plane.  The real axis (SCC) represents coupling strength;
  the imaginary axis (SCD) represents causal direction.
  Red points indicate forward direction ($i\to j$); blue points
  indicate reverse direction.  Most pairs cluster in the lower half-plane
  (SCD $<0$), reflecting Age's dominance as the upstream root node.}
...
\caption{Knowledge quality phase transition (Theorem~\ref{thm:u-shape}).
  ...
  curve.  The empirical curve (blue, 200 trials) closely matches the
  theoretical prediction~$(1-2p)^2 r^*$ (dashed red).  The critical
  threshold~$p^*_{\mathrm{flip}}\approx 0.15$ marks the point below which
  DAG structure ($r_{\mathrm{gradient}}>0.5$) is maintained.  Both
  endpoints ($p=0$ and $p=1$) yield high $r_{\mathrm{gradient}}$
  (correct vs.\ reversed DAG), confirming that partial misinformation
  is worse than complete ignorance.}
\label{fig:pflip}
\end{figure}
