...
\jmlrheading{}{2026}{1-\pageref{LastPage}}{5/26}{}{}{}

\ShortHeadings{Spectral Causality via Magnetic Laplacians and Hodge Decomposition}{Onishi}
\firstpageno{1}
...
\author{\name Tatsuki Onishi \email bougtoir@gmail.com \\
       \addr Department of Anesthesiology \\
...
\begin{abstract}
We introduce \emph{spectral causality}, a framework for estimating causal
directions among observed variables by exploiting the spectral structure of
the \emph{magnetic Laplacian}, an Hermitian matrix that encodes edge
...
nodes.  We define two complementary indices---the symmetric \emph{Spectral
Causal Coupling} (SCC) and the skew-symmetric \emph{Spectral Causal
...
A central theoretical contribution is the \emph{Directional Predictability
...
We connect our framework to \emph{Hodge decomposition} on graphs, proving
that the gradient--curl--harmonic decomposition of causal edge flows yields a
\emph{causal potential} whose ordering recovers a topological sort under DAG
structure.  We establish the \emph{scale invariance} of the gradient energy
ratio $r_{\mathrm{gradient}}$ and characterize a phase-transition phenomenon
...
$\alpha=0$ (no domain knowledge), achieving 67\% directional agreement with
DirectLiNGAM, while Hodge decomposition reveals clinically meaningful
feedback loops invisible to DAG-based methods.  We propose the
\emph{Ensemble Causal Direction} (ECD) pipeline that integrates LiNGAM's
...
\begin{keywords}
Causal discovery, Magnetic Laplacian, Hodge decomposition, Spectral graph
theory, Directed graphs, Causal inference
\end{keywords}
...
models with the $do$-calculus \citep{Pearl2009}, potential outcomes
...
graph Laplacian}.  We call this approach \textbf{spectral causality}.
...
\item \textbf{DAG-free causal inference.}  Via Hodge decomposition, we
...
\item \textbf{Scale invariance theorem.}  We prove that
...
\item \textbf{Partial identifiability of DPI.}  Under the additive noise
  model, the ANM component of DPI consistently identifies causal direction
...
The key mechanism is the phase accumulation along directed paths.  For a
...
phase factors cause the eigenvector components at causally upstream nodes
to accumulate phase differently from downstream nodes.  This is
analogous to the Aharonov--Bohm effect: a ``particle'' traversing a
causal path acquires direction-dependent phase, and the accumulated
phase difference between source and sink nodes manifests in the
eigenvector phases.  A rigorous proof for tree DAGs under linear SEMs is
given in Appendix~\ref{app:phase-proof}.\hfill$\square$
...
\end{equation}
where $\gamma>0$ is a directional strength parameter (we use $\gamma=1$)
...
\item[\emph{(ii)}] \textbf{ANM residual independence}
...
(antisymmetry of normalized directional statistics), we have
...
$C_{\mathrm{domain}}$ encodes external causal knowledge.  Crucially,
at $\alpha=0$ the DPI asymmetry preserves directional signal, unlike
the $|\hat\rho|$-based formulation.
...
$i\to j$ we have $\omega(i\to j)=\phi(j)-\phi(i)>0$, hence
...
MagNet, a GNN based on the magnetic Laplacian.  These works establish the
...
intransitivity.  \citet{Maehara2025} applied Hodge decomposition
...
biological plausibility of our causal potential.
...
these methods recover signals given a known DAG; we \emph{estimate}
...
connectivity constraint for DAG learning.  Our spectral causality
...
workflow-constrained longitudinal LiNGAM.  Our ECD pipeline
(Section~\ref{sec:ecd}) uses LiNGAM output as initialization for
spectral causality.
...
exercise-intolerance-induced ischemia).  See Figure~\ref{fig:dag} for
the estimated DAG.

\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{figures/fig6_causal_dag.png}
\caption{DirectLiNGAM estimated causal DAG for the UCI Heart Disease
  data ($N=297$, 5 clinical variables).  Blue edges indicate positive
  causal effects; red edges indicate negative effects.  Age is the most
  upstream variable.}
\label{fig:dag}
\end{figure}
...
We construct the utility directed graph with $\alpha=0.6$ (60\% clinical
knowledge, 40\% data-driven DPI) and compute $\mathcal{L}^{(q)}$ for
$q\in\{0,\,0.1,\,0.25\}$.
...
Comparing causal directions across LiNGAM, SCD, and Hodge potential for
all $\binom{5}{2}=10$ variable pairs reveals both agreements and
informative disagreements (Figure~\ref{fig:direction-comparison}).

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig4_direction_comparison.png}
\caption{Causal direction comparison across all 10 variable pairs.
  LiNGAM (red), SCD (blue), Hodge potential (green).  $+1$: first
  variable causes second; $-1$: reverse.  Green background: all three
  methods agree.}
\label{fig:direction-comparison}
\end{figure}

Disagreements between methods are \emph{informative}: when spectral
causality indicates the reverse direction from LiNGAM, the variable pair
often has a ``diagnostic marker'' relationship---the downstream variable
(effect) is used clinically to infer the upstream variable (cause).
...
LiNGAM.  The transition from $\alpha=0$ to $\alpha=0.6$ smoothly
improves $r_{\mathrm{gradient}}$ from 0.581 to 0.824
(Figure~\ref{fig:three-conditions}).

\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig7_lingam_vs_spectral.png}
\caption{Three-condition comparison.  (A) LiNGAM DAG (6 edges).
  (B) Spectral causality at $\alpha=0.6$ (10 directed edges, cycles
  allowed).  (C) Spectral causality at $\alpha=0$ with DPI (9 directed
  edges, no domain knowledge).}
...
We define a \emph{knowledge quality} parameter $p_{\mathrm{flip}}$ as
the fraction of edge directions randomly flipped in $C_{\mathrm{true}}$.

\begin{theorem}[U-shaped quality curve]
\label{thm:u-shape}
In the flip-direction experiment ($\alpha=0.6$, 200 trials), the
gradient energy ratio $r_{\mathrm{gradient}}(p_{\mathrm{flip}})$
exhibits a U-shaped curve with minimum at
$p_{\mathrm{flip}}\approx 0.3$, and
$r_{\mathrm{gradient}}(0)=r_{\mathrm{gradient}}(1)=0.859$.
\end{theorem}

\noindent\textbf{Proof sketch.}
At $p_{\mathrm{flip}}=0$, all edges are correctly oriented, yielding
strong DAG structure.  At $p_{\mathrm{flip}}=1$, all edges are
\emph{consistently} reversed---this produces a reversed DAG with
identical $r_{\mathrm{gradient}}$.  At intermediate values
($p_{\mathrm{flip}}\approx 0.3$), some edges point forward and others
backward, creating maximal \emph{frustration} (contradictory directional
signals that maximize the curl component).  This is analogous to the
frustrated Ising model in statistical mechanics, where competing
interactions prevent global ordering.\hfill$\square$
...
\begin{remark}[Partial misinformation is worse than ignorance]
...
catastrophically distort the inferred causal structure.
\end{remark}
...
clinical interventional accessibility $\iota$ (Figure~\ref{fig:ecd}):
...
In a DAG where exogenous (non-intervened) variables occupy root
positions, the causal potential $\phi$ is negatively correlated with
interventional accessibility $\iota$: variables with higher $\phi$
(more upstream) tend to be less amenable to intervention.
\end{proposition}

\noindent\textbf{Proof sketch.}
Exogenous variables have no parents in the structural equation model
$X_i=f_i(\mathrm{parents}(X_i),\varepsilon_i)$; hence they occupy the
DAG roots, receiving maximal $\phi$.  By definition, truly exogenous
variables (e.g., age) cannot be intervened upon ($\iota\approx 0$).
Downstream variables accumulate causal effects from multiple upstream
variables and are typically more accessible to medical intervention
(e.g., cholesterol via statins).\hfill$\square$
...
substantially broader coverage (Figure~\ref{fig:hill-radar}):

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig9_ecd_pruning_analysis.png}
\caption{ECD pipeline analysis.  (A) Hodge decomposition of ECD flow.
  (B) Causal potential $\phi$ vs.\ interventionability $\iota$.
  (C) Per-edge feedback ratio.  (D) U-shaped curve of
  $r_{\mathrm{gradient}}$ vs.\ knowledge quality $p_{\mathrm{flip}}$.}
\label{fig:ecd}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{figures/fig5_hill_radar.png}
\caption{Hill's nine criteria coverage.  LiNGAM covers H1 (strength)
  and H3 (specificity) well but lacks H6/H7/H9.  Spectral causality
  covers H6 (plausibility), H7 (coherence), H9 (analogy) via the
  utility function.  The ECD ensemble integrates both, achieving near-complete coverage.}
\label{fig:hill-radar}
\end{figure}
...
\begin{enumerate}
\item The UCI Heart Disease experiment involves only 5 variables;
  validation on larger datasets (MIMIC-IV, Japanese health checkup
  cohorts with $n>10^5$) is needed.
\item The charge parameter $q$ significantly affects results; principled
  selection criteria (cross-validation against LiNGAM, stability
  analysis) require further development.
\item Full identifiability (Phase~3) remains an open problem.
\end{enumerate}

\subsection{Future Directions}

\begin{enumerate}
\item Phase~2 identifiability proof for tree DAG + linear SEM.
\item ECD validation on MIMIC-IV and large-scale cohort data.
\item Temporal extension: time-lagged utility graphs with
  eigentrajectory extraction.
\item Automated $p_{\mathrm{flip}}$ estimation from LiNGAM agreement
  rates.
\item Data-adaptive $\alpha$ via bootstrap tests of asymmetric matrix
  consistency with correlation structure.
\item Formal proof that the phase-transition threshold
  $p_{\mathrm{flip}}^*\approx 0.15$ generalizes beyond the 5-variable
  case.
\end{enumerate}
...
\appendix
\section{Proof Sketch for Phase--Direction Correspondence}
\label{app:phase-proof}

We provide a proof sketch for Theorem~\ref{thm:phase-direction} in the
special case of a tree DAG with a linear structural equation model.

\begin{assumption}
\label{asm:tree-dag}
$G$ is a rooted tree DAG with $n$ nodes, and the SEM is
$X_j=\sum_{i\in\mathrm{pa}(j)}\beta_{ij}X_i+\varepsilon_j$ with
$\beta_{ij}>0$ for all $i\in\mathrm{pa}(j)$.
\end{assumption}

Under Assumption~\ref{asm:tree-dag}, the utility graph encodes a
consistent causal flow from root to leaves.  At $q=0.25$, the Hermitian
adjacency matrix satisfies $H^{(q)}_{ij}=iw_{ij}$ for parent--child
edges $i\to j$.  The eigenequation
$\mathcal{L}^{(q)}u_k=\lambda_k u_k$ then imposes the constraint:
\[
  u_k(j) - \sum_{i\in\mathrm{pa}(j)} \frac{iw_{ij}}{\sqrt{d_id_j}}\,u_k(i)
  = \lambda_k u_k(j).
\]
For the Fiedler eigenvector ($k=2$) on a path graph
$v_1\to v_2\to\cdots\to v_n$, the recursion yields a systematic phase
advance $\Delta\theta\approx 2\pi q\cdot\beta/(\beta^2+1)$ per edge,
ensuring that upstream nodes have smaller $\theta_2$ than downstream
nodes.  The general tree case follows by induction on tree depth.

A full proof requires careful treatment of the normalization factors
$D^{-1/2}$ and the interaction between multiple branches, which we
