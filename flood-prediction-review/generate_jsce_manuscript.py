...
# 6. 考察
# ============================================================
add_heading_gothic("6. 考察", level=1)

add_heading_gothic("6.1 エネルギー的自立性", level=2)
add_body(
...
add_heading_gothic("6.2 構造的対策との相補性", level=2)
add_body(
...
add_heading_gothic("6.3 限界と課題", level=2)
add_body(
    "本研究にはいくつかの重要な限界がある．"
...
add_heading_gothic("6.4 国際的展開可能性", level=2)
add_body(
...
# 7. 政策提言
# ============================================================
add_heading_gothic("7. 政策提言", level=1)
...
add_body(
    "（1）既存ダムの計画放水と地下水管理を連携させるパイロット事業の実施．"
    "真備地区は合流点付替え事業が完了し，その効果が検証されつつある段階にあり，"
    "追加的な地下水管理型治水の導入を検討する好機である．"
    "（2）帯水層の貯留容量と地盤沈下リスクの詳細調査．"
    "小田川流域および隣接する芦田川水系の沖積層帯水層について，"
    "ボーリングデータと地下水位モニタリングに基づく定量的評価が必要である．"
    "（3）GSFLOW等の統合水文モデルによる詳細シミュレーション{19}．"
    "本研究の簡易モデルを発展させ，地下水–地表水相互作用を動的に評価する必要がある．"
    "（4）「流域治水」政策への地下水管理の制度的位置づけ{22}．"
...
# 8. 結論
# ============================================================
add_heading_gothic("8. 結論", level=1)
...
add_body(
    "（4）日本の急峻な地形，既存の揚水発電インフラ，高密度な気象・水文観測網は，"
